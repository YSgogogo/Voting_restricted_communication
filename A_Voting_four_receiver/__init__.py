...
class Player(BasePlayer):
    ...
    def send_decision_choices(player):
        ...
        if others[0] == others[1]:
            ...
            opts = [f'send to a group member who got {col}',
    ...
    def after_all_players_arrive(self):
        ...
        for g in self.subsession.get_groups():
            ...
            for p in g.get_players():
                seen = p.participant.vars.get('patterns_seen', [])
            ...
            for p in players:
                ...
                p.participant.vars.setdefault('patterns_seen', []).append(chosen)
...
class Info_and_decision(Page):
    ...
    def vars_for_template(player):
        my_img   = 'IndividualDecision/red_urn.png' if player.signals == 'r' \
                   else 'IndividualDecision/blue_urn.png'
        my_grade = 'S' if player.qualities == 'h' else 'W'
        others   = []
        for p in player.group.get_players():
            if p == player:
                continue
            img = 'IndividualDecision/red_urn.png' if p.signals == 'r' \
                  else 'IndividualDecision/blue_urn.png'
            grade = ('S' if p.qualities == 'h' else 'W') \
                    if str(p.id_in_group) in player.info_from_whom.split(',') \
                    else 'Unknown jar'
            others.append(dict(id=p.id_in_group, img_src=img, grade_label=grade))
        return dict(my_img=my_img, my_grade=my_grade,
...
class ResultsWaitPage2(WaitPage):
    ...
    def after_all_players_arrive(self):
        for g in self.subsession.get_groups():
            ...
            # ————— 抽中 receiver —————————
            elif role == 'receiver' and 'do not receive' not in chosen.reveal_decision:
...
class network_and_voting(Page):
    ...
    def vars_for_template(player):
        ...
        rows    = []
        tags    = {p.id_in_group: p.signals + p.qualities
                   for p in player.group.get_players()}

        # 显示规则：pair → 展示自己 & partner 质量；solo-mixed → 只显示自己 signal
        if '+' in pattern:                      # pair
            parts = pattern.split('+')
            for p in player.group.get_players():
                if tags[p.id_in_group] in parts:
                    ...
                    rows.append(dict(id_in_group=p.id_in_group,
                                     is_self=(p == player),
                                     player_signal_style=css,
                                     box_info=('Box A' if p.qualities == 'h'
                                               else 'Box B')))
        elif '_' in pattern:                    # solo-mixed
            p = player
            col = 'red' if p.signals == 'r' else 'blue'
            css = (f'height:1.2em;width:1.2em;background-color:{col};'
                   'border-radius:50%;display:inline-block;'
        ...
        rows.sort(key=lambda d: not d['is_self'])   # 自己排第一
